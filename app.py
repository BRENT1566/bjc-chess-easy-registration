...
def calculate_age_group(dob, year):
    try:
        age = int(year) - datetime.strptime(dob, "%Y-%m-%d").year
    except Exception:
        return "INELIGIBLE"
    if age <= 7: return "U07"
    if age <= 9: return "U09"
...
def save_players_from_form(tournament, year, redirect_endpoint):
    ...
    with db() as con:
        for i in range(max(len(surnames), len(firsts), len(dobs))):
            ...
            section = age_group
            if age_group != "INELIGIBLE":
                section += " Girls" if gender == "Female" else " Boys"
            con.execute(
                "INSERT OR IGNORE INTO submissions (batch_id,school,contact,email,phone,tournament,tournament_year,chessa_id,surname,first_name,birth_date,gender,grade,age_group,section,notes,submitted_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                (batch_id, school, contact, email, phone, tournament, year,
                 ids[i].strip() if i < len(ids) else "", surname, first, dob, gender,
                 grades[i].strip() if i < len(grades) else "", age_group, section,
                 notes[i].strip() if i < len(notes) else "", datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
            )
            saved += 1
    if saved == 0:
...
def validate_grand_challenge_players(division):
    ...
    dobs = request.form.getlist("birth_date[]")
    genders = request.form.getlist("gender[]")
    high_school = "HIGH SCHOOLS" in division
    ...
    for i in range(max(len(surnames), len(firsts), len(dobs), len(genders))):
        ...
        if not surname and not first:
            continue
        if not surname or not first or not dob or gender not in ("Male", "Female"):
            return f"Player row {i+1} is incomplete. Surname, first name, Date of Birth and Gender are compulsory."
        try:
            birth_year = datetime.strptime(dob, "%Y-%m-%d").year
        except ValueError:
            return f"Player row {i+1} has an invalid Date of Birth."

        if high_school and not (2007 <= birth_year <= 2012):
            return f"Player row {i+1} is not eligible for the selected High Schools division. High Schools players must be born from 2007 to 2012."
        if not high_school and birth_year < 2013:
            return f"Player row {i+1} is not eligible for the selected Primary Schools division. Primary Schools players must be born in 2013 or later."
    return None
...
def register():
    if request.method == "POST":
        ...
        try:
            year = int(request.form.get("tournament_year", "2026"))
        except ValueError:
            year = 2026
...
def west_coast_grand_challenge():
    ...
    if request.method == "POST":
        ...
        if division not in GRAND_CHALLENGE_DIVISIONS:
            ...
            return redirect(url_for("west_coast_grand_challenge"))
        validation_error = validate_grand_challenge_players(division)
        if validation_error:
...
def admin():
    ...
    with db() as con:
        rows = con.execute("SELECT * FROM submissions ORDER BY section,school,surname,first_name").fetchall() if section == "ALL" else con.execute("SELECT * FROM submissions WHERE section=? ORDER BY school,surname,first_name", (section,)).fetchall()
        summary = con.execute("SELECT section,COUNT(*) c FROM submissions GROUP BY section ORDER BY section").fetchall()
...
def export_csv():
    if not require_admin():
        return redirect(url_for("admin_login"))
    with db() as con:
        rows = con.execute("SELECT * FROM submissions ORDER BY section,school,surname,first_name").fetchall()
    out = io.StringIO()
    w = csv.writer(out)
    w.writerow(["School Name", "Contact Teacher / Coach", "Email Address", "Cell Number", "Tournament / Event", "Tournament Year", "CHESSA ID", "Surname", "First Name", "Date of Birth", "Gender", "Grade", "Age Group", "Section", "Notes"])
    for r in rows:
        w.writerow([r["school"], r["contact"], r["email"], r["phone"], r["tournament"], r["tournament_year"], r["chessa_id"], r["surname"], r["first_name"], r["birth_date"], r["gender"], r["grade"], r["age_group"], r["section"], r["notes"]])
    data = "\ufeff" + out.getvalue()
    filename = f"BJC_ONLINE_REGISTRATIONS_{datetime.now():%Y%m%d_%H%M}.csv"
...
def clear_all():
    ...
    flash("All online registrations have been cleared.")
    return redirect(url_for("admin"))
